            ...
            <div style={{backgroundColor:'rgb(0 0 0 / 17%)'}} className="py-12 transition duration-150 ease-in-out z-10 absolute top-0 right-0 bottom-0 left-0 W-57" id="modal">
                <div role="alert" style={{maxWidth:'700px'}} className="container mx-auto w-11/12 md:w-2/3 max-w-lg">
                    <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                        ...
                        <>
                        <h1 className="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">{sharableChat[0].name}</h1>
                        <div
                            id="chat"
                                    ...
                                    sharableChat[0].user_chats.map((item,i)=>(
                                        <React.Fragment key={i}>
                                            <div className='flex justify-end relative'>
                                                ...
                                                <div>
                                                <div className='chat_userName_2 text-right'>{JSON.parse(localStorage.getItem("data")).name}</div>
                                                    ...
                                                    >
                                                        ...
                                                        <span className="block text-xs text-right" style={{fontSize:'0.5rem'}}>{item.created_at.slice(11,16)}</span>
                                                    </div>
                                                </div>
                                                </div>
                                            </div>
                        ...
                        <div className='bg-gray-100 text-start text-black px-4 mb-4'>
                            {JSON.parse(localStorage.getItem("data")).name} at {sharableChat[0].created_at.slice(0,10)}
                        </div>
                        <div className="flex items-center justify-between w-full">
                            <label className="relative inline-flex cursor-pointer items-center">
                                <input id="switch" type="checkbox" className="peer sr-only" defaultChecked  onChange={(e) => {e.target.checked ? handleCopyShare("yes"): handleCopyShare("no") }}/> {/* setShareName(e.target.checked ? "yes" : "no") */}
                            ...
                            <button onClick={copyLink} className="text-slate-800 hover:text-blue-600 text-sm bg-white hover:bg-slate-100 border border-slate-200 rounded-l-lg font-medium px-4 py-2 inline-flex space-x-1 items-center">
                                <span>
                                ...
                                <span>{copyAction ? "copied successfully" : "Copy Link"}</span>
                            </button>
                        ...
                        <button onClick={()=>setShareToggle(false)} className="cursor-pointer absolute top-0 right-0 mt-4 mr-5 text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out rounded focus:ring-2 focus:outline-none focus:ring-gray-600" onclick="modalHandler()" aria-label="close modal" role="button">
